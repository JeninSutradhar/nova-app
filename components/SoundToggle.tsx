...
export function SoundToggle() {
  ...
  function toggleSound() {
    ...
    setMuted(newMuted);
    localStorage.setItem('nova-sound-muted', JSON.stringify(newMuted));
    
    // Play a test sound if unmuting
    if (!newMuted) {
      // Use setTimeout to ensure the localStorage is updated before playing
      setTimeout(() => {
  ...
  return (
    <motion.button
      ...
      whileTap={{ scale: 0.95 }}
      onClick={toggleSound}
      className="rounded-full bg-white/10 p-2 backdrop-blur border border-white/20 hover:bg-white/20 transition-colors"
